...
## Corpus Check
- 1 files · ~22,895 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 398 nodes · 621 edges · 42 communities (23 shown, 19 thin omitted)
...
4. `_sample_png_bytes()` - 11 edges
5. `ConvertParams` - 9 edges
6. `convert_to_ansi_grid()` - 9 edges
7. `reportCrash()` - 8 edges
8. `compressImageIfNeeded()` - 8 edges
9. `convert_ascii()` - 8 edges
10. `convert_ansi()` - 8 edges
...
- `ControlsBarProps` --references--> `AnsiPalette`  [EXTRACTED]
  components/ControlsBar.tsx → lib/types.ts
- `ControlsBarProps` --references--> `OutputMode`  [EXTRACTED]
  components/ControlsBar.tsx → lib/types.ts
...
Cohesion: 0.09
Nodes (29): OutputHeader(), OutputHeaderProps, TABS, tabStyle(), parseNum(), SessionLoader(), SessionLoaderProps, VersionFooter() (+21 more)
...
Cohesion: 0.18
Nodes (14): DownloadPage(), FALLBACK_ASSETS, fetchRelease(), DownloadLinks(), Props, Footer(), LINK_STYLE, OutputCanvas (+6 more)
...
## Knowledge Gaps
- **120 isolated node(s):** `FeedbackPayload`, `BugPayload`, `BugReportPayload`, `FALLBACK_ASSETS`, `metadata` (+115 more)
...
- **What is the exact relationship between `Vercel Frontend Deployment` and `vercel.svg (Vercel triangle logo)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `MockImage` connect `Client Image Compression` to `Image Converters (Server)`?**
...
- **Should `Server Deploy Docs` be split into smaller, more focused modules?**
